...
class VpnView(QWidget):
    ...
    def _build_ui(self):
        ...
        self._tor_v4_lbl  = self._ip_row(ip_l, 'Exit  IPv4:', '#00cc33')
        self._tor_v6_lbl  = self._ip_row(ip_l, 'Exit  IPv6:', '#00cc33')
        _note = QLabel('Exit IP is sampled — each app gets its own Tor circuit.')
        _note.setStyleSheet('font-size: 8px; color: #888;')
        ip_l.addWidget(_note)
